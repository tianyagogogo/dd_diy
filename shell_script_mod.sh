...
function diycron(){
    ...
    ln -sf /scripts/docker/docker_entrypoint.sh /scripts/docker/docker_entrypoint_mix.sh
    echo "22 */2 * * * /scripts/docker/docker_entrypoint_mix.sh >> /scripts/logs/default_task.log 2>&1" >> /scripts/docker/merged_list_file.sh
    # 整点京豆雨
